...
// Helper function to redact sensitive data from API responses
function redactSensitive(obj: any): any {
  ...
  const sensitiveKeys = [
    ...
    'idToken', 'authorization', 'authorizationHeader', 'session', 'secret',
...
export default function TestAuthPage() {
  ...
  const testAuth = async () => {
    ...
    try {
      ...
      if (response.ok) {
        setTestResult(`✅ SUCCESS: ${JSON.stringify(redactSensitive(data), null, 2)}`)
      } else {
  ...
  const testUserCreation = async () => {
    ...
    try {
      ...
      if (response.ok) {
        setTestResult(`✅ USER CREATED: ${JSON.stringify(redactSensitive(data), null, 2)}`)
