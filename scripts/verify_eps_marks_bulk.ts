...
// Note: Using ILIKE matching in script so exact casing isn't critical, 
// but good to know specific names from DB:
  ...
  for (const subjectName of subjects) {
    ...
    for (const sequence of SEQUENCES) {
      ...
      if (assessments && assessments.length > 0) {
        // Check grades for the first matching assessment found
