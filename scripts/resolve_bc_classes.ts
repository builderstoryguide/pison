/**
 * Resolve Form 1–5 BC class IDs and verify class_subjects.
 * Run: npx tsx scripts/resolve_bc_classes.ts
 */
...
async function main() {
  ...
  const BC = classGroups.filter(
    (g) => g.dbSearchName.includes('BC') && g.dbSearchName.startsWith('Form')
  )
  ...
  for (const g of BC) {
    ...
    const missing = expected.filter(
      (e) => !names.some((n) => n.toLowerCase() === e.toLowerCase())
    )
