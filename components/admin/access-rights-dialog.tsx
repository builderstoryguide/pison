...
export function AccessRightsDialog({ user, open, onOpenChange }: AccessRightsDialogProps) {
  const { updateUserAccessRights, getAvailablePermissions, isLoading } = useUserManagement()
  ...
  const loadAvailablePermissions = async () => {
    ...
    try {
      const permissions = await getAvailablePermissions(user.role)
  ...
  const handleSave = async () => {
    try {
      const success = await updateUserAccessRights(user.id, selectedPermissions)
      if (success) {
        toastSuccess('Success', `Access rights updated for ${user.name}`)
        onOpenChange(false)
      } else {
        toastError('Error', 'Failed to update access rights')
      }
    } catch (error) {
  ...
  const getPermissionDescription = (permission: string): string => {
    const descriptions: Record<string, string> = {
      ...
      'mark_attendance': 'Record and manage student attendance',
          ...
          {isLoadingPermissions ? (
            ...
            <div className="space-y-6">
              ...
              <Alert>
                ...
                <AlertDescription>
                  <strong>Note:</strong> Only permissions appropriate for the {user.role} role are shown. 
                  Some permissions may be restricted based on the user's role for security reasons.
                </AlertDescription>
              </Alert>
            </div>
          )}
